...
def workflow(verbose, fh_input, col_csv, col_dict, force):
    ...
    if verbose:
        print("XML field values:")
        print('\t'.join(cols))  ### printing here as a header for the output
        if col_dict:
            print("REPLACED, desired header line values:")
            print('\t'.join(col_renames))


    value_dict = {}  ### col (field) names vs the value we find associated
    ...
    for line in fh_input:
        ...
        for col in cols:  # the input list that we are looking for
            if col == l0[1]:  ### if one of our query cols matches exactly the field in the line...
                found = True
                val = l0[0]

                value_dict[col] = val  ### make the assignment, which will UPDATE previous values
                if verbose:
                    print("*** found: ", col, l0, val)
        if verbose:
            print(value_dict)
    ...
    printlist = []
    for col in cols:
        if value_dict.get(col):
            printlist.append(value_dict.get(col))
        else:
            printlist.append("NA")
